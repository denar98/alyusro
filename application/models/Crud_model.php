...
class Crud_model extends CI_Model
{
	...
	{
		parent::__construct();
		if (file_exists(APPPATH . 'config/crud.php') and $this->config->load('crud', TRUE)) {
			$this->initialize($this->config->item('crud'));
		}
	...
	public function readData($select, $from, $wheres = NULL, $joinTable = NULL, $groupBy = NULL, $orderBy = NULL, $limit = NULL)
	{
		...
		if (is_array_assoc($joinTable)) {
			$this->set_joins($joinTable);
		}
		if (is_array_assoc($wheres)) {
			$this->set_wheres($wheres);
		} else if (is_string($wheres) and trim($wheres) !== '') {
			$this->db->where($wheres);
		}
		if ($this->delete_record === FALSE) {
			$this->db->where($from . '_delete_date', NULL);
		}
		if (is_array($groupBy)) {
			$this->db->group_by($groupBy);
		} else if (is_string($groupBy) and trim($groupBy) !== '') {
			$this->db->group_by($groupBy);
		}
		if (is_string($orderBy) and trim($orderBy) !== '') {
			$this->db->order_by($orderBy);
		}
		if (is_array($limit) and count($limit) <= 2) {
			if (is_numeric($limit[0]) and !empty($limit[1]) and is_numeric($limit[1])) {
				$this->db->limit($limit[0], $limit[1]);
			} else if (is_numeric($limit[0])) {
	...
	public function readDataTable($select, $from, $wheres = NULL, $joinTable = NULL, $groupBy = NULL, $output = 'full')
	{
		...
		for ($a = 0; $a < count($columns); $a++) {
			...
			if ($aa === FALSE) {
				if (isset($column[$columns[$a]['data']])) {
					$aa = $columns[$a]['data'];
				}
			}
		...
		for ($b = 0; $b < count($order); $b++) {
			...
			if ($bb === FALSE) {
				if (isset($column[$columns[$ba['column']]['data']])) {
					$bb = $columns[$ba['column']]['data'];
				}
			}
			if ($columns[$ba['column']]['orderable'] === 'true' && is_numeric($bb)) {
				$this->db->order_by($selects[$bb], $ba['dir']);
			}
		}
		$sqlData = '';
		if ($length > 0) {
		...
		foreach ($rData as $rDataKey => $rDataVal) {
			foreach ($this->dtColumnCreate as $dtcck => $dtccv) {
				$rData[$rDataKey][$dtcck] = $this->exec_replace($dtccv, $rData[$rDataKey], $cols);
			}
			foreach ($this->dtColumnUpdate as $dtcuk => $dtcuv) {
				if (array_key_exists($dtcuk, $rData[$rDataKey])) {
					$rData[$rDataKey][$dtcuk] = $this->exec_replace($dtcuv, $rData[$rDataKey], $cols);
				}
			}
			foreach ($this->dtColumnDelete as $dtcd) {
				if (array_key_exists($dtcd, $rData[$rDataKey])) {
					unset($rData[$rDataKey][$dtcd]);
				}
			}
			if (is_numeric($columns[0]['data'])) {
				$rData[$rDataKey] = array_values($rData[$rDataKey]);
			}
		}
		$return = array();
		if ($output === 'full') {
			...
			if ($length > 0 && $sqlData !== '') {
				...
				$return['recordsFiltered'] = $qData->num_rows();
			}
	...
	{
		if (trim($var) !== '') {
			$this->db->insert('log', array('log_ip' => $this->input->ip_address(), 'log_query' => $var, 'log_url' => (!isset($_SERVER['REDIRECT_URL'])) ? base_url() : $_SERVER['REDIRECT_URL'], 'log_datetime' => date('Y-m-d H:i:s')));
		}
	}
		...
		if (in_array_assoc($this->wheres, $arr)) {
			...
			foreach ($arr as $keys => $vals) {
						...
						if (is_array_assoc($vals)) {
							...
							foreach ($vals as $val) {
								if (is_array($val)) {
									...
									$this->db->$key($val, NULL, FALSE);
								}
							}
						} else if (is_string($vals)) {
							$this->db->$key($vals, NULL, FALSE);
						}
					} else {
						if (is_array_assoc($vals)) {
							foreach ($vals as $k => $v) {
				...
				if ($groupIdx === 1 && isset($this->group_end[$groupKey]) && is_numeric($this->group_end[$groupKey])) {
					for ($a = 0; $a < (int) $this->group_end[$groupKey]; $a++) {
						$this->db->group_end();
					}
				}
			}
